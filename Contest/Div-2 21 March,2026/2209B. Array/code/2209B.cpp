...
//
//   For each index i, find the maximum number of indices j such that j > i
//   and |a[i] − k| > |a[j] − k|, over all possible integer values of k.
//
// Input:
//   The first line contains an integer t (1 ≤ t ≤ 100) — number of test cases.
//   For each test case:
//     - First line: integer n (1 ≤ n ≤ 5000)
//     - Second line: n integers a1, a2, ..., an (−1e9 ≤ ai ≤ 1e9)
//
// Output:
//   For each test case, print n integers — the answer for each index i.
//
// Constraints:
//  - 1 ≤ t ≤ 100
//  - 1 ≤ n ≤ 5000
...
void solve()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        int base = 0; // initially satisfied LEFT events
        ...
        for(int j= i+1 ; j<n; j++)
        {
            ...
            if(a[j] > a[i])
            {
                b.push_back({mid, +1}); // RIGHT event
            }
            else if(a[j] < a[i])
            {
                b.push_back({mid, -1}); // LEFT event
                base++; // initially satisfied
            }
        }
...
/*
   Solution Logic:
  - Fix an index i and consider all j > i.
  - The condition |a[i] - k| > |a[j] - k| depends on k and changes at midpoint values.
  - For each pair (i, j), define a critical value (a[i] + a[j]).
  - Treat each pair as an "event":
      * If a[j] < a[i], it contributes initially (base++) and later becomes invalid.
      * If a[j] > a[i], it becomes valid after crossing midpoint.
  - Sort all events based on midpoint.
  - Sweep through events:
      * Update current count using +1 or -1 transitions.
      * Track maximum value.
  - Output the maximum count for each i.

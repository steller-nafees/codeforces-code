//============================================================================
// Platform      : Codeforces 
// Problem Title : 16B. Burglar and Matches
// Author        : H M Nafees N Islam
// Institution   : North South University (NSU), Computer Science And Engineering
// Date          : January 15, 2026 
//
// Problem Statement:
//   A burglar got into a matches warehouse and wants to steal as many matches
//   as possible. In the warehouse there are m containers, where the i-th
//   container contains ai matchboxes, and each matchbox contains bi matches.
//   All matchboxes are of the same size.
//
//   The burglar's rucksack can hold exactly n matchboxes. Since he has no
//   time to rearrange matches inside boxes, he must choose at most n
//   matchboxes such that the total number of matches is maximized.
//
// Input:
...
//
// Output:
//   Print a single integer — the maximum number of matches the burglar
//   can carry.
//
// Constraints:
//  - 1 ≤ n ≤ 2 × 10^8
...
void solve()
{
    ...
    pair_input_vec(matches, m);

    sort_pair_second_desc(matches);
    ...
    for(int i=0; i<m ; i++)
    {
        if(rem <= matches[i].first)
        {
            totalMatches += rem * matches[i].second;
        }
        else
        {
            totalMatches += matches[i].first * matches[i].second;
        }
        rem -= matches[i].first;
        if(rem <= 0 ) break;
    }
...
/*
   Solution Logic:
   - Store all containers as pairs (ai, bi), where:
       ai = number of matchboxes
       bi = matches per matchbox
   - Sort the containers in descending order of bi to prioritize boxes
     with the highest number of matches.
   - Greedily take as many matchboxes as possible from each container,
     without exceeding the rucksack capacity n.
   - For each container:
       • Take min(remaining_capacity, ai) boxes
       • Add (taken_boxes × bi) to the total matches
   - Stop when the rucksack is full.

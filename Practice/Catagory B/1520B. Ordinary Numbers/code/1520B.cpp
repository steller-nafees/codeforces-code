//============================================================================
// Platform      : Codeforces 
// Problem Title : 1520B. Ordinary Numbers
// Author        : H M Nafees N Islam
// Institution   : North South University (NSU), Computer Science And Engineering
// Date          : February 01, 2026 
//
// Problem Statement:
// An integer is called an "ordinary number" if all of its digits are the same.
// For example, 1, 2, 9, 11, 22, 33, 444, 5555 are ordinary numbers.
//
// You are given an integer n. For each test case, determine how many ordinary
...
//
// Output:
// For each test case, print the number of ordinary numbers that are
...
void solve()
{
    ...
    int res = (len - 1) * 9; // base count with fewer digits

    char firstDig = s[0];

    string repeated(len, firstDig);
    if(stoll(repeated) <= n)
    {
        res += (firstDig - '0');
    }
...
/*
   Solution Logic:
  - Ordinary numbers consist of repeated digits only.
  - For numbers with fewer digits than n, there are exactly 9 ordinary numbers
    for each digit length.
  - For numbers with the same length as n, construct a number using the first
    digit repeated and compare it with n.
  - If it is ≤ n, include all such ordinary numbers up to that digit.
  - Otherwise, exclude the last one.
  - This approach runs in O(1) per test case.
*/

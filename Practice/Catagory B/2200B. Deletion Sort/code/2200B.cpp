//============================================================================
// Platform      : Codeforces 
// Problem Title : 2200B. Deletion Sort
...
/*
  ...
  - If the array is already non-decreasing, then no deletions are required.
    So the minimum remaining elements = n.

  - If the array is NOT non-decreasing, then there exists at least one pair
    such that v[i] > v[i+1].

  - Since we are allowed to remove ANY element in each turn, we can keep
    removing elements until only one element remains.

  - A single element array is always non-decreasing by definition.

  - Therefore:
        If array is non-decreasing → answer = n
        Otherwise → answer = 1

  Time Complexity per test case: O(n)
  Overall Complexity: O(t × n)

  Since n ≤ 10, this is extremely efficient.
*/

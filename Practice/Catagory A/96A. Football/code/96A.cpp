...
//   To simplify the situation he depicted it as a string consisting of 
//   zeroes and ones.
//
//   A zero corresponds to players of one team; a one corresponds to players 
//   of another team. If there are at least 7 players of some team standing 
//   one after another, then the situation is considered dangerous.
//
//   For example:
//     - 00100110111111101 → dangerous
//     - 11110111011101 → not dangerous
//
//   Determine whether the given situation is dangerous or not.
//
// Input:
...
void solve()
{
    ...
    cout<<(OUTPUT_CONDITION(danger)) << endl;
}
...
/*
   Solution Logic:
   - Read the binary string representing player positions.
   - Keep track of consecutive identical characters (either '0' or '1').
   - Initialize count = 1 and set the first character as the current team.
   - Traverse the string from index 1 to n-1:
       • If current character equals previous team → increment count.
       • Otherwise → reset count to 1 and update team.
   - If count reaches 7 at any point → mark situation as dangerous.
   - Finally:
       • Print "YES" if dangerous.
       • Otherwise print "NO".
   - Time Complexity: O(n)

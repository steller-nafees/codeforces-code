//============================================================================
// Platform      : Codeforces 
// Problem Title : 318A. Even Odds
// Author        : H M Nafees N Islam
// Institution   : North South University (NSU), Computer Science And Engineering
// Date          : March 11, 2026 
//
// Problem Statement:
//   Being a nonconformist, Volodya is displeased with the current order of
//   natural numbers (positive integers). He decides to rearrange them.
//   Since there are infinitely many natural numbers, he starts with the
//   first n numbers.
//
//   He writes them in a new order:
//     1. First all odd numbers from 1 to n in ascending order.
//     2. Then all even numbers from 1 to n in ascending order.
//
//   After forming this sequence, we need to determine which number
//   will appear at position k.
//
// Input:
//   A single line containing two integers n and k.
//
// Output:
//   Print the number that appears at position k in the rearranged sequence.
//
// Constraints:
//   - 1 ≤ k ≤ n ≤ 10^12
//   - Time limit: 1 second
...
/*
   Solution Logic:
  - The sequence is rearranged so that all odd numbers from 1 to n appear first,
    followed by all even numbers from 1 to n.

  - Total odd numbers in the range [1, n] = (n + 1) / 2.

  - If k is within the range of odd numbers:
        The k-th position corresponds to the k-th odd number.
        k-th odd number = 2*k - 1.

  - Otherwise:
        The element lies in the even part of the sequence.
        First subtract the number of odd elements:
            new position = k - (n + 1) / 2
        new_position-th even number = 2 * new_position.

  - Since n can be as large as 10^12, generating the sequence is impossible,
    so we compute the result mathematically in O(1) time.
*/
